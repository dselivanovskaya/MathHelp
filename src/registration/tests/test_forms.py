...
class SignUpFormTest(TestCase):
    ...
    def test_form_is_invalid_if_username_is_not_available(self):
        form = SignUpForm({
            'username': JOHN_USERNAME, 'email': ALICE_EMAIL,
    ...
    def test_form_is_invalid_if_email_is_not_available(self):
        form = SignUpForm({
            'username': ALICE_USERNAME, 'email': JOHN_EMAIL,
            'password1': ALICE_PASSWORD, 'password2': ALICE_PASSWORD,
        })
        self.assertFalse(form.is_valid())
    ...
    def test_form_is_invalid_if_passwords_dont_match(self):
        form = SignUpForm({
            ...
            'password1': ALICE_PASSWORD, 'password2': ALICE_PASSWORD + '0',
        })
        self.assertFalse(form.is_valid())
    ...
    def test_form_is_invalid_if_password_doesnt_have_one_digit(self):
        form = SignUpForm({
            'username': ALICE_USERNAME, 'email': ALICE_EMAIL,
            'password1': 'aliceiscool', 'password1': 'aliceiscool',
        })
        self.assertFalse(form.is_valid())
    ...
    def test_form_is_invalid_if_password_doesnt_have_one_special_character(self):
        form = SignUpForm({
            'username': ALICE_USERNAME, 'email': ALICE_EMAIL,
            'password1': 'aliceIsCool12', 'password2': 'aliceIsCool12',
        })
        self.assertFalse(form.is_valid())
    ...
    def test_form_displays_correct_error_message_if_password_doesnt_have_one_digit(self):
        response = self.client.post(self.view_url, {
            'username': ALICE_USERNAME, 'email': ALICE_EMAIL,
            'password1': 'aliceiscool', 'password2': 'aliceiscool',
        })
        self.assertFormError(response,
